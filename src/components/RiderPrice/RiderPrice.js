  ...
  const ridePrice = (
    <>
      ...
      <div className="mb-3 bg-white p-3 rounded">
        <img
          style={{ width: "60px" }}
          src={rider.imgURL ? rider.imgURL : data[0].imgURL}
          alt=""
        />{" "}
        <p className=" d-inline ml-3 mr-2">
          {" "}
          {rider.name ? rider.name.toLowerCase() : data[0].name.toLowerCase()}
        </p>{" "}
        ...
        <strong>
          ...
          <p className="d-inline">
            {" "}
            {rider.capacity ? rider.capacity : data[0].capacity}
          </p>
        </strong>{" "}
        <span className="ml-5"> ${rider.price}</span>
      </div>
    </>
  );
  console.log(rider);
    ...
    <div className="container d-flex flex-column flex-md-row pt-5 mt-2 border-top justify-content-center">
      <div className="rider-price">
        <div className="fromToLocation p-3 text-white rounded ">
          <h3>
            <span style={{ color: "red", marginRight: "8px" }}>From</span>
            {location.from}
          </h3>
          <h3 className="mt-3">
            <span style={{ color: "red", marginRight: "8px" }}>To</span>
            {location.to}
          </h3>
        </div>
        {ridePrice}
